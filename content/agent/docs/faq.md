...
a = "Yes — the `blockchain` tool group adds read-only EVM and Substrate/Polkadot tools (balances, blocks, transactions, contract calls, ENS, storage queries). It's compiled in when the daemon is built with the `blockchain` cargo feature (enabled in the release binaries) and activated per session with `load_tools blockchain`. See the [tools reference](@/agent/docs/tools.md#blockchain-tools)."
