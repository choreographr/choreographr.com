...
description = "Choreographr v0.1.0 is out — an open-source (Apache-2.0) AI agent in Rust with a sandboxed RISC-V VM, a persistent server, and 70+ model providers."
date = 2026-08-13
...
registered, logged tools. Around that core sits a **persistent server** that
...
- **A server that owns your sessions** — clients come and go; start a task in
...
Here's what that looks like in practice. I asked the agent to use the
`riscv-vm` tool — the prompt below is the one I typed into `choreo-tui`:

> Use the riscv-vm tool to run a rust program that programatically generates a
> beautiful, detailed and colorful geometric design in a SVG and show it to me.
> Only use integer math.

The agent writes a small Rust program, the server compiles it to RISC-V and
runs it inside the sandboxed VM, and the generated SVG comes back as a file the
TUI renders inline:

![choreo-tui: the riscv-vm prompt typed into the session](/blog/2026-08-12-choreographr-0-1-0/riscv-vm-01-prompt.webp)

![choreo-tui: the agent writing the Rust program](/blog/2026-08-12-choreographr-0-1-0/riscv-vm-02.webp)

![choreo-tui: compiling and running the program in the RISC-V VM](/blog/2026-08-12-choreographr-0-1-0/riscv-vm-03.webp)

![choreo-tui: the generated SVG rendered inline in the terminal](/blog/2026-08-12-choreographr-0-1-0/riscv-vm-04.webp)

![choreo-tui: the finished session](/blog/2026-08-12-choreographr-0-1-0/riscv-vm-05.webp)

And this is the SVG the VM produced — a geometric mandala drawn entirely with
integer math:

![Geometric mandala SVG generated by a Rust program running in the RISC-V VM](/blog/2026-08-12-choreographr-0-1-0/geometric-mandala.svg)

## One server, every surface

Most agents are a single process with a single UI. Choreographr is built the
other way around: one **server** owns your sessions, and clients connect and
...
Because sessions live in the server, you can start a task in the terminal and
check on it from your phone. Everything persists in an embedded `redb` database
and survives restarts.

<!-- TODO: add a short screencast of streaming here (and a click-to-open
fullscreen view of an inline image) once available -->

![Choreographr architecture: clients (TUI, GUI, IM, ACP) and the server handoff connect to the choreographr server, which talks to model providers and runs MCP servers, a RISC-V VM sandbox, and a redb database](/architecture.svg)
...
- **Memory safety by construction** — no null dereferences, use-after-free, or
  ...
  server that holds your keys and runs untrusted model output should not be one
  segfault away from a problem.
- **Threads, not async** — the server uses real OS threads with message
...
- **70+ model providers** — OpenAI, Anthropic, Mistral, DeepSeek, xAI, Groq,
  ...
  Gemini APIs.
- **Concurrent sessions & subsessions** — the server runs many sessions in
